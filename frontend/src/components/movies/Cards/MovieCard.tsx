...
export function MovieCard({ id, title, poster_path, vote_average, horizontal, moviesCount }: MovieCardProps) {
    ...
    if (horizontal) {
      return (
        <Card onClick={(e) => handleCardClick(e)} className="flex flex-row w-full h-[140px] rounded-xl overflow-hidden shadow-md bg-white dark:bg-zinc-900">
          <img src={poster_path} alt={title} className="w-[100px] h-full object-cover" />
          ...
          <div className="flex flex-col justify-between p-4 flex-1">
            <div>
              ...
              <div className="flex items-center gap-1 text-yellow-400 mt-1">
                ...
                <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{vote_average.toFixed(1)}</span>
              </div>
            </div>
    ...
    return (
      <Card onClick={(e) => handleCardClick(e)} className="w-[200px] h-[500px] flex flex-col justify-between rounded-2xl overflow-hidden shadow-md transition-transform duration-300 hover:scale-[1.03] hover:shadow-lg bg-white dark:bg-zinc-900">
        <img src={poster_path} alt={title} className="w-full h-[320px] object-cover" />
